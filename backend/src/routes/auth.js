...
router.post('/register', async (req, res) => {
  try {
    ...
    const user = db.query('SELECT id, email, nickname, created_at FROM users WHERE email = ?', [email]).rows[0];
...
router.post('/login', async (req, res) => {
  try {
    ...
    if (!email || !password) return res.status(400).json({ error: 'Email and password required' });
    const result = db.query('SELECT id, email, password_hash, nickname, is_admin FROM users WHERE email = ?', [email]);
...
router.post('/refresh', authMiddleware, async (req, res) => {
  try {
    const user = db.query('SELECT id, email, nickname, is_admin FROM users WHERE id = ?', [req.userId]).rows[0];
